			...
			<div class="card-body">
				<nav class="nav nav-pills nav-justified">
					...
					<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_HRADMIN')">
						<a class="nav-item nav-link active" href="${cPath}/admin/setHrManager">인사 관리자</a>
					</security:authorize>
				...
				<form action="${cPath}/admin/updateHrAdmin" method="post" class="row mt-5">
					<div class="col-5">
					...
					<div class="col-5">
						<c:set var="hrAdminList" value="${hrAdminList}" />
						<h6>인사 관리자</h6>
						<select name="adminMemId" id="search_to" class="form-control" size="8" multiple="multiple">
							<c:if test="${not empty hrAdminList }">
								<c:forEach items="${hrAdminList }" var="hrAdmin">
									<option value="${hrAdmin.memId}">${hrAdmin.memName}(${hrAdmin.memId})-${hrAdmin.psName}/${hrAdmin.deptName}
										<c:forEach items="${hrAdmin.roleInfoList}" var="roleInfo">
											${roleInfo.roleName }
										</c:forEach>
									</option>
								</c:forEach>
