...
<div class="page-header">
	<div>
		<h2 class="main-content-title tx-24 mg-b-5" id="pageName">전사 파일보관함</h2>
		<ol class="breadcrumb">
		</ol>
	</div>
</div>
		...
		<div class="card custom-card overflow-hidden">
			<div class="card-body">
				...
				<div class="card custom-card mr-2" style="display: inline;">
					<input class="btn btn-outline-success" type="button" id="createFolderBtn" value="새폴더" />
					<input class="btn btn-outline-success" type="button" id="downloadBtn" value="다운로드" />
					<input type="button" id="btnCompose" class="btn btn-outline-primary" value="메일보내기">
					<input class="btn btn-outline-secondary" type="button" id="deleteFileBtn" value="삭제" />
				</div>
				<!-- 상단 메뉴바 END -->
				<br/>
				<br/>
				...
				<div id="createFolderDiv" class="d-flex p-1 mb-2 newFolder hide">
					<form:form commandName="fileBoxVO" id="createForlderForm" action="${cPath }/fileBox/public/folderInsert">
						<input type="hidden" id="fileType" value="PUBLIC"/>
...
<script type="text/javascript">
// 	var fileType = "${pagingVO.fileType }";
</script>
						...
						if(file.fileDir == 'Y'){
							trTags.push(
								$("<tr>").append(
									...
									, $("<td>").append(
										$("<a href='javascript:void(0);' class='infolder'  data-fileno='"+file.fileNo+"' data-parentno='"+file.fileParent+"' data-fname='"+file.fileRealName+"'>").append(
											$("<i>").attr("class", "fa fa-folder-open").prop("aria-hidden",true)
											,$("<span>").text(file.fileRealName)
											, $("<input type='text'>").attr({value:file.fileRealName, name:"fileRealName"}).css("display", "none")
										)
										, $("<input type='button'>").attr({"class":"btn btn-outline-light btn-sm ml-3 folderNameUpdateBtn", value:"폴더수정"})
									)
							...
							trTags.push(
								$("<tr>").append(
									$("<td>").append(
										$("<input type='checkbox' data-dir='N'>").css({width:"20px", height:"20px"}).attr("name","fileNo")
									).attr("class", "text-center")	
									, $("<td>").append(
										$("<span>").text(file.fileRealName)
										, $("<input type='button'>").attr({"class":"btn btn-outline-light btn-sm ml-3", value:"다운로드"})
									)
									, $("<td>").append(
