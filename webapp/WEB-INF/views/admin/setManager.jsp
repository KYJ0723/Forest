...
* Copyright (c) 2021 by DDIT All right reserved
 ...
 <div class="page-header">
	<div>
		<h2 class="main-content-title tx-24 mg-b-5">시스템 설정</h2>
		<ol class="breadcrumb">
		...
		<div class="card custom-card">
			...
			<div class="card-body">
				<nav class="nav nav-pills nav-justified">
					<a class="nav-item nav-link active" href="${cPath}/admin/setManager">전체관리자</a>
					<a class="nav-item nav-link" href="${cPath}/admin/setHrManager">인사 관리자</a>
					<a class="nav-item nav-link" href="${cPath}/admin/setEaManager">전자결재 관리자</a>
					<a class="nav-item nav-link" href="${cPath}/admin/setCmManager">커뮤니티 관리자</a>
				</nav>

				<form action="${cPath}/admin/updateSuperAdmin" method="post" class="row mt-5">
					<div class="col-5">
					...
					<div class="col-5">
						<c:set var="superAdminList" value="${superAdminList}" />
						<h6>슈퍼 관리자 (최대 5명)</h6>
						<select name="adminMemId" id="search_to" class="form-control" size="8" multiple="multiple">
							<c:if test="${not empty superAdminList }">
								<c:forEach items="${superAdminList }" var="superAdmin">
									<option value="${superAdmin.memId}">${superAdmin.memName}(${superAdmin.memId})-${superAdmin.psName}/${superAdmin.deptName}
										<c:forEach items="${superAdmin.roleInfoList}" var="roleInfo">
											${roleInfo.roleName }
										</c:forEach>
									</option>
								</c:forEach>
