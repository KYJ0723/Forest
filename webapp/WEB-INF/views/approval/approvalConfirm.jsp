...
<div class="page-header">
	<div>
		...
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active" aria-current="page">확인</li>
		</ol>
	</div>
</div>
		...
		<div class="card custom-card">
			<div class="card-header border-bottom-0 pb-0">
				<div class="d-flex justify-content-between">
					<label class="main-content-label mb-0 pt-1">확인</label>
				</div>
			</div>
			<div class="card-body">
				<table class="table text-nowrap text-md-nowrap table-bordered mg-b-0" id="detail">
					...
					<thead class="text-center">
						<tr>
							...
							<th style="font-size:1.2rem;" onclick="event.cancelBubble=true">
								<span>
									<spring:message code="approval.result" />
								</span>
							</th>
						</tr>
					</thead>
					<tbody class="text-center">
						<c:set var="approvalConfirmList" value="${pagingVO.dataList }" />
						<c:if test="${not empty approvalConfirmList }">
							<c:forEach items="${approvalConfirmList }" var="approval">
								<tr>
									<td>${approval.adNo }</td>
									...
									<td>${approval.dsResult }</td>
								</tr>
							</c:forEach>
						</c:if>
						<c:if test="${empty approvalConfirmList }">
							<tr>
