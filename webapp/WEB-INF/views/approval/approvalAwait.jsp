	...
	<div>
		...
		<ol class="breadcrumb">
			<li class="breadcrumb-item">
				<a href="${cPath}/approval/approvalAwait">진행중인 문서</a>
			</li>
			<li class="breadcrumb-item active" aria-current="page">대기</li>
		</ol>
	</div>
</div>
			...
			<div class="card-body">
				<table class="table text-nowrap text-md-nowrap table-bordered mg-b-0" id="detail">
					...
					<tbody class="text-center">
						<c:set var="approvalAwaitList" value="${pagingVO.dataList }" />
						<c:if test="${not empty approvalAwaitList }">
							<c:forEach items="${approvalAwaitList }" var="approval" varStatus="status">
								<tr>
									<td>${approval.adNo }</td>
									...
									<td>${approval.aprvlStateCode }</td>
								</tr>
							</c:forEach>
						</c:if>
						<c:if test="${empty approvalAwaitList }">
							<tr>
