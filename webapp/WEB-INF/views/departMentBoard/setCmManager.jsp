...
<div class="page-header">
	<div>
		<h2 class="main-content-title tx-24 mg-b-5">커뮤니티</h2>
		<ol class="breadcrumb">
			...
			<div class="card-body">
				<nav class="nav nav-pills nav-justified">
					...
					<security:authorize access="hasAnyRole('ROLE_ADMIN,ROLE_CMADMIN')">
						<a class="nav-item nav-link active" href="${cPath}/admin/setCmManager">커뮤니티 관리자</a>
					</security:authorize>
				</nav>

				<form action="${cPath}/admin/updateCmAdmin" method="post" class="row mt-5">
					<div class="col-5">
					...
					<div class="col-5">
						<c:set var="cmAdminList" value="${cmAdminList}" />
						<h6>게시판 관리자</h6>
						<select name="adminMemId" id="search_to" class="form-control" size="8" multiple="multiple">
							<c:if test="${not empty cmAdminList }">
								<c:forEach items="${cmAdminList }" var="cmAdmin">
									<option value="${cmAdmin.memId}">${cmAdmin.memName}(${cmAdmin.memId})-${cmAdmin.psName}/${cmAdmin.deptName}
										<c:forEach items="${cmAdmin.roleInfoList}" var="roleInfo">
											${roleInfo.roleName }
										</c:forEach>
									</option>
								</c:forEach>
