 ...
 * </pre>
 */ 
const APPROVALPROGRESSLISTTABLE = $('#approvalProgressListTable'); 

const listBody = $("#listBody");


$.approvalProgressList=function(){
	$.ajax({
		url : $.getContextPath() + "/approval/progress.do",
		data:{memId : memId},
		dataType : "json",
		success : function(resp) {
			...
			let table = APPROVALPROGRESSLISTTABLE.DataTable({
				paging : true,
				search : true,
			...
			.buttons().container().appendTo("#approvalProgressListTable_wrapper .col-md-6:first");
		}
	}); // ajax end
